...
public class Display {
		...
		for (int i = 0; i < numrows; i++) {
			for (int j = 0; j < numcols; j++) {
				...
				if (piece != 0) {
					p.fill(0);
					p.textSize(60);
					p.text(piece, x + j * dx + dx / 2 - 80, y + i * dy + dy / 2 + 15);
				}
			}
		}
	}
	 ...
	 * 
	 * @param pieceType The type of piece in the grid.
	 * @param filename  The file path to the image to be used for the given type of
	 *                  piece.
	 */
	...
	public void highlightLocation(Location l, TF8class g) {
		if (g.isInGrid(l)) {
	...
	public void initializeWithGame(TF8class game) {
